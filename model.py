...
class GInNER(nn.Module):
  def __init__(self, word_embedding_dim, device, dropout, hidden_layer, nheads):
    ...
    self.dropout = dropout
    self.device = device
    self.gat = GAT(nfeat=self.input_size, nhid=self.hidden_layer, nclass=8, dropout=self.dropout, nheads=self.nheads, alpha=0.2)
